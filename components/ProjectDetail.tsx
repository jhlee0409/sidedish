...
import { ArrowLeft, ExternalLink, Heart, Calendar, User, Share2, Hash, MessageCircle, Send, Sparkles, Lock, MessageSquareMore, Smartphone, Gamepad2, Palette, Globe, Box, Github, Download, ChefHat, Utensils } from 'lucide-react';
...
const ProjectDetail: React.FC<ProjectDetailProps> = ({ project, onBack }) => {
  // Local state for interactivity within the session
  const [reactions, setReactions] = useState<{ [key: string]: number }>(project.reactions || {});
  ...
  const handleCommentSubmit = (e: React.FormEvent) => {
    ...
    const comment: Comment = {
      ...
      author: 'Guest Gourmet', // Mock user
      content: newComment,
      createdAt: new Date(),
    };
  ...
  // Determine CTA Button Content based on Platform
  const getCTAContent = () => {
  ...
  return (
    <div className="container mx-auto px-4 max-w-5xl animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <button 
        onClick={onBack}
        className="group flex items-center gap-2 text-slate-500 hover:text-orange-600 mb-8 transition-colors font-medium"
      >
        <div className="p-2 rounded-full bg-white border border-slate-200 group-hover:border-orange-100 group-hover:bg-orange-50 transition-all">
      ...
      <div className="space-y-6 mb-10">
        ...
        <div className="flex flex-wrap items-center gap-4 text-slate-500 text-sm md:text-base border-b border-slate-100 pb-8">
          ...
          <div className="flex items-center gap-1 text-red-500 font-medium">
             ...
             <span>{project.likes}명이 입맛을 다셨습니다</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 lg:gap-16 pb-20">
        ...
        {/* Left Column: Content, Reactions, Comments */}
