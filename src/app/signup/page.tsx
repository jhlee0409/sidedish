...
export default function SignupPage() {
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      // 로그인 성공 후 needsProfileSetup이 true면 프로필 설정 폼 표시
      // false면 dashboard로 리다이렉트 (useEffect에서 처리)
    } catch (err) {
  ...
  const handleGithubSignIn = async () => {
    try {
      ...
      setError('GitHub 회원가입에 실패했습니다. 다시 시도해주세요.')
      console.error(err)
    } finally {
      setIsLoading(false)
    }
          ...
          <div className="bg-white rounded-3xl shadow-xl shadow-slate-200/50 overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="p-8 space-y-4">
              ...
              <div className="bg-orange-50 rounded-xl p-4 mb-6">
                <h3 className="font-semibold text-orange-800 mb-2">SideDish 회원 혜택</h3>
